...
static void ionic_dim_work(struct work_struct *work)
{
	...
	if (intr->dim_coal_hw != new_coal) {
		...
		intr->dim_coal_usecs = cur_moder.usec;
...
static int ionic_alloc_qcq_interrupt(struct ionic_lif *lif, struct ionic_qcq *qcq)
{
	...
	if (affinity_mask_override) {
		...
		cpumask_copy(*affinity_mask, cpu_none_mask);
		...
		irq_set_affinity_hint(qcq->intr.vector, *affinity_mask);

	} else if (cpumask_empty(*affinity_mask)) {
		...
		if (qcq->intr.cpu != -1)
			cpumask_set_cpu(qcq->intr.cpu, *affinity_mask);
	}
			   ...
			   unsigned int pid, struct ionic_qcq **qcq)
{
	...
	*qcq = NULL;
	...
	new->q.info = vzalloc_node(num_descs * desc_info_size,
				   dev_to_node(dev));
	if (!new->q.info) {
...
static void ionic_qcq_sanitize(struct ionic_qcq *qcq)
{
	qcq->q.tail_idx = 0;
	qcq->q.head_idx = 0;
	qcq->q.exp_comp_idx = 0;
	qcq->q.exp_comp_color = 1;
	qcq->cq.tail_idx = 0;
	qcq->cq.done_color = 1;
...
static int ionic_lif_rxq_init(struct ionic_lif *lif, struct ionic_qcq *qcq)
{
	...
	else if (lif->xdp_prog->aux->xdp_has_frags)
		ctx.cmd.q_init.flags |= cpu_to_le16(IONIC_QINIT_F_SG);
#endif
				  ...
				  netdev_features_t features)
{
	...
	if (lif->phc)
		ctx.cmd.lif_setattr.features |=
			le64_to_cpu(lif->ionic->ident.lif.eth.config.features) &
			(IONIC_ETH_HW_TIMESTAMP | IONIC_ETH_HW_RDMA_TIMESTAMP);
...
static int ionic_txrx_alloc(struct ionic_lif *lif)
{
	...
	for (i = 0; i < lif->nxqs; i++) {
		...
		if (flags & IONIC_QCQ_F_INTR) {
			...
			if (test_bit(IONIC_LIF_F_TX_DIM_INTR, lif->state)) {
				...
				lif->txqcqs[i]->intr.dim_coal_usecs = lif->tx_coalesce_usecs;
			}
		}
	...
	for (i = 0; i < lif->nxqs; i++) {
		...
		if (test_bit(IONIC_LIF_F_RX_DIM_INTR, lif->state)) {
			...
			lif->rxqcqs[i]->intr.dim_coal_usecs = lif->rx_coalesce_usecs;
		}
			       ...
			       int vf, struct ifla_vf_info *ivf)
{
	...
	if (vf >= pci_num_vf(ionic->pdev) || !ionic->vfs) {
		...
		ivf->vlan_proto	  = swab16(vfdata->vlanproto);
...
#ifndef IONIC_HAVE_NDO_VF_VLAN_PROTO
...
#endif
{
	...
	struct ionic *ionic = lif->ionic;
	bool use_vlan_ext_attr;
	int ret;

	if (vlan > 4095)
		return -EINVAL;

	use_vlan_ext_attr = (ionic->ident.dev.capabilities &
			     cpu_to_le64(IONIC_DEV_CAP_VF_VLAN_EXT));

	if ((!use_vlan_ext_attr && qos) || qos > 7)
		return -EINVAL;
	...
	if (vf >= pci_num_vf(ionic->pdev) || !ionic->vfs) {
		...
		if (use_vlan_ext_attr) {
			vfc.attr = IONIC_VF_ATTR_VLAN_EXT;
			vfc.vlan_ext.id = cpu_to_le16(vlan);
			vfc.vlan_ext.proto = swab16(proto);
			vfc.vlan_ext.qos = qos;
		} else {
		...
		dev_dbg(ionic->dev,
			"%s: vf %d vlan %d qos %d proto 0x%04x\n",
			__func__, vf, vlan, qos, be16_to_cpu(proto));

		ret = ionic_set_vf_config(ionic, vf, &vfc);
		if (!ret) {
			ionic->vfs[vf].vlanid = cpu_to_le16(vlan);
			if (use_vlan_ext_attr) {
				ionic->vfs[vf].vlanproto = swab16(proto);
				ionic->vfs[vf].qos = qos;
			}
		}
	}
...
#define XDP_ERR_MTU "MTU is too large for XDP without frags support"
	...
	b_maxfs = IONIC_XDP_MAX_LINEAR_MTU;
#ifdef HAVE_NET_XDP_FRAGS
	if (bpf->prog && bpf->prog->aux && bpf->prog->aux->xdp_has_frags)
		b_maxfs = maxfs;
#endif
	if (bpf->prog)
		maxfs = min_t(u32, b_maxfs, maxfs);
...
static int ionic_lif_adminq_init(struct ionic_lif *lif)
{
	...
	for (i = 0; i < q->num_descs; i++)
		init_completion(&q->admin_info[i].work);
