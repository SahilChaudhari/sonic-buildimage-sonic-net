...
#define GVE_DRIVER_NAME		"gve-mnic"
#define GVE_LAN_DRIVER_NAME	"gve-lan-mnic"
...
static void mdev_uio_remove(struct platform_device *pdev)
#else
...
/* Note that these gve_mnic BAR resources are
 * in a different order than the ionic_mnic BAR
 */
#define IONIC_INTR_MSIXCFG_STRIDE	0x10
#define GVE_VECTOR_COUNT		31
#define GVE_REG_SIZE		64
#define GVE_INTR_CTRL_SIZE	(GVE_VECTOR_COUNT * IONIC_INTR_MSIXCFG_STRIDE)
#define GVE_DOORBELL_SIZE	(GVE_VECTOR_COUNT * sizeof(u32))
#define GVE_MSIX_SIZE		(GVE_VECTOR_COUNT * IONIC_INTR_MSIXCFG_STRIDE)

static int mdev_get_gve_platform_rsrc(struct platform_device *pdev,
				      struct mdev_create_req *req)
{
	struct resource mnet_resource[] = {
		{ /*reg_bar*/
			.flags    = IORESOURCE_MEM,
			.start    = req->regs_pa,
			.end      = req->regs_pa + GVE_REG_SIZE - 1,
		}, {/*intr_ctrl_bar*/
			.flags    = IORESOURCE_MEM,
			.start    = req->drvcfg_pa,
			.end      = req->drvcfg_pa + GVE_INTR_CTRL_SIZE - 1,
		}, {/*db_bar*/
			.flags    = IORESOURCE_MEM,
			.start    = req->doorbell_pa,
			.end      = req->doorbell_pa + GVE_DOORBELL_SIZE - 1,
		}, {/*msix_bar*/
			.flags    = IORESOURCE_MEM,
			.start    = req->msixcfg_pa,
			.end      = req->msixcfg_pa + GVE_MSIX_SIZE - 1,
		},
	};
			   ...
			   unsigned int cmd)
{
	...
	case MDEV_CREATE_GVE:
	case MDEV_CREATE_GVE_LAN:
		if (req->is_uio_dev)
			pdev->driver_override = kasprintf(GFP_KERNEL, "%s", UIO_DRIVER_NAME);
		else if (cmd == MDEV_CREATE_GVE_LAN)
			pdev->driver_override = kasprintf(GFP_KERNEL, "%s", GVE_LAN_DRIVER_NAME);
		else
			pdev->driver_override = kasprintf(GFP_KERNEL, "%s", GVE_DRIVER_NAME);
		err = mdev_get_gve_platform_rsrc(pdev, req);

		/* gve-mnic needs the req->name for the netdev name */
		platform_device_add_data(pdev, req->name, MDEV_NAME_LEN + 1);
		break;
...
static inline bool mdev_ioctl_matches(struct mdev_dev *mdev, unsigned int cmd)
{
	...
	if (cmd == MDEV_CREATE_GVE && mdev->type == MDEV_TYPE_MNET)
		return true;

	if (cmd == MDEV_CREATE_GVE_LAN && mdev->type == MDEV_TYPE_MNET)
		return true;
