...
def fetch_dpu_files():
    ...
    platform = api_helper_platform if api_helper_platform != None else "arm64-salina_leni-r0"
...
def get_platform_type():
    """
    Detect platform type from machine.conf or device directory.
    Returns 'salina' for Salina/Leni platform, 'elba' otherwise.
    """
    platform = None
    
    # Try to get platform from machine.conf
    for conf_path in ["/etc/machine.conf", "/host/machine.conf"]:
        try:
    ...
    # Check if it's Salina platform
    if platform and "salina" in platform.lower():
        return "salina"
...
def set_ubootenv_config():
    """
    Configure fw_env.config for U-Boot environment access.
    
    For Salina: Uses eMMC partition 11 with redundant environment
    For Elba: Uses MTD device for ubootenv
    """
    platform_type = get_platform_type()
    
    if platform_type == "salina":
        # Salina uses eMMC partition 11 for U-Boot environment with redundancy
        # Format: device offset env_size sector_size
        # Primary at offset 0x0, Redundant at offset 0x1000
        salina_fw_env_pn = 11
        fw_env_config_primary = "/dev/mmcblk0p{} 0x0 0x1000 0x200".format(salina_fw_env_pn)
        fw_env_config_redund = "/dev/mmcblk0p{} 0x1000 0x1000 0x200".format(salina_fw_env_pn)
        with open("/etc/fw_env.config", "w") as f:
            f.write(fw_env_config_primary + "\n")
            f.write(fw_env_config_redund + "\n")
        log_info("Configured fw_env.config for Salina (eMMC p{}) with redundancy".format(salina_fw_env_pn))
    else:
        # Elba uses MTD device for U-Boot environment
        ...
        log_info("Configured fw_env.config for Elba (MTD{})".format(mtd_ubootenv))
